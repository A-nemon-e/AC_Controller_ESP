...
public:
    ...
    void init() {
        // 从DisplayConfig加载配置
        DisplayConfig::init();
        DisplaySettings settings = DisplayConfig::getSettings();
        
        cardCount = 0;
        currentCardIndex = settings.currentCard;
        autoSwitchCards = false;  // 默认关闭自动切换
        cardSwitchInterval = settings.switchInterval * 1000;
        ...
        // 从配置设置转场效果
        transition.setType(static_cast<TransitionType>(settings.transitionType));
        transition.setDuration(DEFAULT_TRANSITION_DURATION);
        
        // 设置全局亮度
        LEDMatrix::setGlobalBrightness(settings.brightness);
        
        memset(cards, 0, sizeof(cards));
        
        Serial.printf("[DisplayManager] 初始化完成 - 亮度:%d 自动切换:%s 间隔:%d秒\n",
                      settings.brightness, 
                      autoSwitchCards ? "是" : "否",
    ...
    void switchCard(size_t index) {
        ...
        if (transition.getType() != TransitionType::INSTANT) {
            ...
            // 初始化新卡片的背景
            if (getCurrentCard() && getCurrentCard()->getBackground()) {
                getCurrentCard()->getBackground()->init();
            }
