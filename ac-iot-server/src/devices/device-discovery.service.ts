...
export class DeviceDiscoveryService {
    ...
    if (topic.startsWith('ac/discovery/')) {
      ...
      try {
        ...
        if (!data.uuid || !data.mac) {
          this.logger.warn(
            `[Discovery] ❌ 无效消息 (缺uuid/mac): ${payloadStr}`,
          );
          return;
        }
        ...
        if (!data.userId || uid === 0) {
          ...
          if (this.discoveredDevices.has(data.uuid)) {
            ...
            this.logger.log(
              `[Discovery] ℹ️ 设备 ${data.uuid} 已绑定 (ID: ${data.userId})，移除`,
            );
          } else {
            this.logger.log(
              `[Discovery] ℹ️ 忽略已绑定设备: ${data.uuid} (ID: ${data.userId})`,
            );
          }
        }
      } catch (error) {
        this.logger.error(
          `[Discovery] ❌ 解析失败: ${error.message} \nPayload Hex: ${payload.toString('hex')}`,
        );
      }
    }
  }
  ...
  getAvailableDevices(maxAgeMinutes: number = 5): DiscoveredDevice[] {
    ...
    this.logger.log(
      `[Discovery] API请求获取设备列表。当前缓存总数: ${devices.length}, maxAge: ${maxAgeMinutes}min`,
    );
    ...
    const validDevices = devices.filter((device) => {
      ...
      this.logger.debug(
        `[Discovery] 检查设备 ${device.uuid}: age=${age}ms, valid=${isValid} (lastSeen: ${device.lastSeen.toISOString()})`,
      );
      return isValid;
    });
  ...
  private async pushRecoveryConfig(device: Device, mac: string) {
    ...
    if (device.brandConfig) {
      try {
        ...
        this.logger.log(
          `[Recovery] 推送品牌配置到 ${topicConfig}: ${brandPayloadStr}`,
        );
      } catch (e) {
