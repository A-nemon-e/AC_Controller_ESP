...
export class DevicesService {
  ...
  async create(
    ...
    if (device) {
      this.logger.log(
        `Device ${createDeviceDto.uuid} already exists, updating ownership (Re-claim)`,
      );
      // 更新所有权
  ...
  async remove(userId: number, id: number): Promise<void> {
    ...
    this.logger.log(
      `Unbinding device ${device.uuid}: sent config reset to ${topic}`,
    );
  ...
  async sendCommand(userId: number, deviceId: number, command: AcCommandDto) {
    ...
    if (device.brandConfig) {
      try {
        ...
        if (brandCfg.brandId) {
          ...
          this.logger.log(
            `[Brand Protocol] Device ${deviceId} using brand ${brandCfg.brandId}, sending semantic command`,
          );
        ...
        this.logger.warn(
          `Failed to parse brandConfig for device ${deviceId}: ${e.message}`,
        );
      }
    }
    ...
    if (strategy !== 'brand_protocol' && device.irConfig) {
      ...
      if (key && device.irConfig[key]) {
        ...
        this.logger.log(
          `[Custom IR] Found learned code for ${key}, sending RAW data`,
        );
    ...
    this.logger.log(
      `Sending command to ${topic} (strategy: ${strategy}): ${payloadString}`,
    );
  ...
  async setBrand(
    ...
    this.logger.log(
      `Device ${deviceId} brand set to ${brandId} ${model || ''}, config sent via MQTT`,
    );
    return device;
  }
    ...
    try {
      ...
      if (topic.endsWith('/brands/list')) {
        ...
        if (brands.length > 0) {
          ...
          await this.devicesRepository.save(device);
          this.logger.log(
            `Updated supportedBrands for ${uuid}: ${brands.length} protocols`,
          );
        }
      }
      ...
      // 3. 处理 learn/result
        ...
        this.logger.log(
          `Learn Result for ${uuid} (key=${data.key}): ${data.success}`,
        );
      }
      ...
      // 4. ✅ 处理 status 状态上报 (Fixing Sync Issue)
        ...
        if (!packet?.retain && !device.isOnline) {
          ...
          this.logger.log(
            `Device ${uuid} marked ONLINE via implicit status heartbeat`,
          );
          device.lastSeen = new Date(); // Update check-in time
        ...
        device.lastState = {
          ...
          mode: data.mode,
          setTemp:
            data.setTemp || data.targetTemp || device.lastState?.setTemp || 26, // Robust fallback
  ...
  private displayConfigs = new Map<number, any>();

  /**
   * 获取设备显示配置
   */
  async getDisplayConfig(userId: number, deviceId: number) {
    const device = await this.devicesRepository.findOne({
      where: { id: deviceId },
    });
    if (!device) throw new NotFoundException('Device not found');
    ...
    // 返回存储的配置或默认配置
    const config = this.displayConfigs.get(deviceId) || {
      autoSwitch: true,
      switchInterval: 30,
      transition: 'fade',
      brightness: 150,
   ...
   * 更新设备显示配置
   */
  async updateDisplayConfig(userId: number, deviceId: number, dto: any) {
    const device = await this.devicesRepository.findOne({
      where: { id: deviceId },
    });
    if (!device) throw new NotFoundException('Device not found');
    ...
    // 获取现有配置
    const existingConfig = this.displayConfigs.get(deviceId) || {
      autoSwitch: true,
      switchInterval: 30,
      transition: 'fade',
      brightness: 150,
    ...
    const newConfig = {
      ...existingConfig,
      ...dto,
      cards: dto.cards || existingConfig.cards,
    };

    // 存储配置
    this.displayConfigs.set(deviceId, newConfig);

    // 通过MQTT推送到设备
    const topic = `ac/user_${userId}/dev_${device.uuid}/display/config`;
    const payload = JSON.stringify(newConfig);
    this.mqttService.publish(topic, payload);
    this.logger.log(
      `Updated display config for device ${deviceId}, pushed to ${topic}`,
    );

    return { success: true, deviceId, config: newConfig };
  }
}
