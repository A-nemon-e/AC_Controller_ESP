...
export class RoutinesService implements OnModuleInit {
  ...
  async evaluateRules(
    deviceId: number,
    sensorData: { temp?: number; hum?: number },
  ) {
    ...
    for (const routine of routines) {
      ...
      if (allMatch && triggers.length > 0) {
        this.logger.log(
          `Routine "${routine.name}" triggered by Sensor Rule for Device ${deviceId}`,
        );
  ...
  private async executeRoutineActions(
    routine: Routine,
    triggeringDeviceId?: number,
  ) {
    ...
    for (const action of routine.actions) {
      ...
      if (!targetDevId) {
        this.logger.warn(
          `Routine "${routine.name}" action missing target device ID`,
        );
        continue;
      }
  ...
  async update(
    userId: number,
    id: number,
    updateData: Partial<CreateRoutineDto>,
  ): Promise<Routine> {
