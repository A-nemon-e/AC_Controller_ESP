...
export class SchedulesController {
  ...
  async update(
    ...
    const existingRoutine = await this.routinesService.findOne(
      req.user.userId,
      id,
    );
    ...
    const routineData = this.mapScheduleToRoutine(
      mergedSchedule as CreateScheduleDto,
    );

    // Update
    const updated = await this.routinesService.update(
      req.user.userId,
      id,
      routineData as any,
    );
  ...
  private mapRoutineToSchedule(routine: Routine) {
    ...
    const action =
      routine.actions && routine.actions.length > 0
        ? routine.actions[0].cmd
        : {};
    const deviceId =
      routine.actions && routine.actions.length > 0
        ? routine.actions[0].deviceId
        : 0;
