...
export class UplinkService {
  ...
  async handleMqttMessage(event: { topic: string; payload: Buffer }) {
    ...
    try {
      ...
      this.logger.debug(
        `Router: Type=${type}, User=${userId}, UUID=${deviceUuid}`,
      );
  ...
  private async handleIREvent(device: Device, data: any) {
    ...
    this.logger.debug(
      `Device ${device.id} received unknown IR signal: ${data.protocol}`,
    );
  }
  ...
  private async handleAutoDetectResult(device: Device, data: any) {
    if (data.success && data.isAC) {
      ...
      this.logger.log(
        `Device ${device.id} auto-detected: ${data.protocol} model ${data.model}`,
      );
      ...
      // ❌ 识别失败
      this.logger.warn(
        `Device ${device.id} auto-detect failed (${data.protocol})`,
      );
